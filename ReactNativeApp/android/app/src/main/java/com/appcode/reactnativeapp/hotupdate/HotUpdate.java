...
public class HotUpdate {
    ...
    public static void mergePatAndAsset(Context context) {
        ...
        String assetsBundle = FileUtils.getJsBundleFromAssets(context,FileConstant.JS_BUNDLE_LOCAL_FILE);
        // 2.解析bundle当前目录下.pat文件字符串
        String patcheStr = FileUtils.getJsBundleFromAssets(context,FileConstant.PATCHES_FILE);
        // 3.合并
    ...
    private static void merge(String patcheStr, String bundle) {
        ...
        // 6.保存新的bundle文件
        try {
            String path = FileConstant.JS_BUNDLE_LOCAL_PATH.substring(0,FileConstant.JS_BUNDLE_LOCAL_PATH.lastIndexOf("/"));
